...
<head>
    ...
    <script type="text/javascript">
        ...
        function awardSummary() {
            document.searchForm.action = "${ctx}/terminal/exchangeAward/summary";
            document.searchForm.submit();
            document.searchForm.action = "${ctx}/terminal/exchangeAward/";
        }
    </script>
...
<form:form id="searchForm" name="searchForm" action="${ctx}/terminal/exchangeAward/" method="post"
    ...
    <div class="col-md-10">
        ...
        <input id="summary" onclick="awardSummary()" class="btn btn-primary" type="button" value="汇总"/>
    </div>
</form:form>
    ...
    <c:forEach items="${page.list}" var="mac">
        <tr>
            ...
            <td>${fns:getDictLabels(mac.isWinPrize, 'isWinPrize', '')}</td>
                ...
                <%--兑奖金额 --%>
            <td style="text-align: center;"><fmt:formatNumber value="${mac.cashPrizeMoney}" type="currency"/></td>
                <%--计奖金额 --%>
            <td style="text-align: center;"><fmt:formatNumber value="${mac.calcPrizeMoney}" type="currency"/></td>
                <%--大小奖 --%>
            <c:choose>
