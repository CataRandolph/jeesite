...
//            var isWinPrize = $("#isWinPrizeId").val();
		...
		function summary(){
			document.searchForm.action="${ctx}/terminal/ticket/summary";
			document.searchForm.submit();
			document.searchForm.action="${ctx}/terminal/ticket/";
		}
	</script>
	...
	<form:form id="searchForm" name="searchForm" modelAttribute="ticketInfo" action="${ctx}/terminal/ticket" method="post" class="breadcrumb form-search">
	    ...
	    <div class="col-md-10">
			...
			<li>
				...
				<input id="sumId" onclick="summary()" class="btn btn-primary" type="button" value="汇总"/>
			</li></br>
		</div>
	</form:form>
	...
	<table id="contentTable" style="TABLE-LAYOUT: fixed" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th style="text-align: center;" width="100">计奖金额</th>
		...
		<c:forEach items="${page.list}" var="ticket">
			<tr>
				...
				<td style="text-align: center;"><fmt:formatNumber value="${ticket.billMoeny}" type="currency"/></td>
				...
				<td style="text-align: center;"><fmt:formatNumber value="${ticket.cashPrizeMoney}" type="currency"/></td>
				<td style="text-align: center;"><fmt:formatNumber value="${ticket.calcPrizeMoney}" type="currency"/></td>
